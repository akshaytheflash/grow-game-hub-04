...
SELECT c.id, title, content, order_index, duration_minutes
...
) AS lessons(title, content, order_index, duration_minutes)
...
SELECT c.id, name, description, icon, color
FROM public.chapters c,
(VALUES 
...
) AS badges(name, description, icon, color),
(VALUES 
...
) AS chapters_list(title)
WHERE c.title = chapters_list.title;